# JSON Studio — Performance Patterns

Doc này note lại các pattern + lesson learned khi tune performance cho tool `/json-studio` (formerly `/json-viewer`). Đặc biệt với JSON lớn (>100KB) và case user paste / select-all / paste liên tục.
...
### [fail] Force remount qua `key={dataVersion}`
...
### [fail] 2-step null-render giữa unmount + mount
...
### [fail] Terminate parser worker mỗi setData
...
### [fail] Web Worker để parse off-thread
...
|---|---|---|
| Text editor | `jsonStudioEditorStore` | Mỗi keystroke (sync) |
| Committed data (cho view) | `jsonStudioStore` | Debounced 400ms |
| Preferences (theme, ruler...) | `jsonStudioPrefsStore` | Khi user toggle (rare) |

→ Subscribe đúng store thì component không re-render thừa.

### 2. GPU compositing cho camera animation

`src/lib/json-studio/JSONCrackStyles.module.css`:
...
### [fail] Đừng dùng `key={dataVersion}` để force remount Canvas
...
### [fail] Đừng spinner overlay với `backdrop-filter: blur(...)`

Backdrop-filter full-screen tốn CPU mỗi frame. Cạnh tranh với ELK → spinner giật.

## Bundle size

Lazy route `/json-studio`, chunk ~2.6MB (gzip ~840KB). Heavy deps:
...
- [ ] `npm run build` chunk `/json-studio` không quá to (~2.6MB raw OK)
