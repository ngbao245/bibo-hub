# BiBo Library Tool — Tổng Hợp Chức Năng

> Rename từ Reader → Library (2026-07-08, spec `library-shared`). Tool giờ mount ở `/library/*` với thư viện chung shared cho mọi user login. Highlight/progress vẫn per-user. File source đã chuyển `src/routes/reader/` → `src/routes/library/` (analog cho `components/` + `lib/` + `api/`). Naming component nội bộ (`PdfReader`, `ReaderHeader`, `ReaderSidebar`) giữ nguyên vì mô tả chức năng, không phải label tool.
...
**Entry Point:** `src/routes/library/Reader.tsx` → route-level wrapper
...
### File: `src/components/library/PdfReader.tsx` (main component, ~1000 LOC)
...
### File: `src/components/library/SelectionMenu.tsx`
...
### SettingsDropdown (`src/components/library/SettingsDropdown.tsx`)
...
### ReaderSidebar (`src/components/library/ReaderSidebar.tsx`)
...
### PDF Search (`src/lib/library/pdf-search.ts`)
...
```ts
{
  ...
  cover_url?: string;
  uploaded_by_id: string;   // UUID audit — user upload sách
  uploaded_by: string | null; // Username hiển thị dưới cover, null với data cũ
  // ... other fields
}
```

Note: field `user_id` legacy đã rename thành `uploaded_by_id` (spec `library-shared`). RLS Project A cho phép mọi authenticated SELECT/INSERT/DELETE — permission chỉ enforce ở tầng frontend (admin-only cho delete button).

Data source: sau spec `library-migrate-to-project-a`, mọi bảng (`books`, `highlights`, `reading_progress`) + storage bucket `books` nằm ở Project A (`bibo-tools-auth`) — cùng project với auth/profiles/app_settings. Library dùng chung `authClient` qua re-export `@/lib/library/supabase`. Project B (`hubibo`) giữ nguyên snapshot cho rollback, không code nào query.
...
4 tính năng quản lý sách + notes ở `/library` route level (không phải Reader):

### 14.1 Export ghi chú

Menu 3 chấm trên book card → **Xuất ghi chú** → download JSON `{tên-sách}-notes.json`. Format `NotePackV1` (schema versioning ở `src/lib/library/note-pack.ts`):
...
Toolbar Library có nút **Xuất tất cả ghi chú** → ZIP chứa N file JSON (mỗi sách 1 file, skip sách không có note để zip gọn).

**RLS mặc định**: chỉ export highlight/progress của user hiện tại. Không leak data user khác.

### 14.2 Nhập ghi chú

Chỉ per-book (nút "Nhập ghi chú" trong dropdown 3 chấm). Bỏ nút Import global ở toolbar — user luôn chọn target sách trước.

Dialog flow:
1. Chọn file `.json` hoặc `.zip`.
2. Parse + validate schema.
3. Match book: `book_id` first → fallback fuzzy title (edit distance < 3).
4. Dedupe theo key `(book_id | user_id | page | text-first-100)` — import lại pack đã import = 0 imported, N skipped.
5. Upsert progress theo `(book_id, user_id)`.

**Cross-user import**: `user_id` bị override sang user hiện tại (RLS `with_check user_id = auth.uid()`). Ownership "gốc" không lưu — YAGNI cho single-user setup.

### 14.3 Thay file PDF (admin only)

Menu 3 chấm → **Thay file PDF** → dialog upload file mới. Giữ nguyên `book_id`, `highlights`, `reading_progress` — chỉ đổi `file_path`, `file_url`, `cover_url` trong bảng `books`.

Flow (mutation `useReplaceBookFile` ở `src/api/library/books.ts`):
1. Compress qua iLovePDF nếu 2MB ≤ size ≤ 30MB.
2. Upload file mới + cover mới lên path mới.
3. `UPDATE books SET file_path=..., cover_url=...` với `WHERE id = book.id`.
4. Xoá file cũ + cover cũ khỏi Storage (**sau** UPDATE thành công, tránh mất data nếu DB fail).
5. Rollback: UPDATE fail → xoá file mới upload, giữ file cũ nguyên.

Warning % page diff (`getPdfPageCount` từ `pdf-page-count.ts`):
- < 5%: an toàn.
- 5-20%: warning vàng, user vẫn OK confirm.
- > 20%: warning đỏ + require checkbox "Tôi hiểu rủi ro" trước khi enable nút Replace.

Highlight rects lưu **normalized** (0-1 %) nên resize page không lệch. Nhưng nếu file mới có số trang khác nhiều → highlight rơi vào trang sai. Đó là rủi ro user phải chấp nhận.

### 14.4 Xoá sách (admin only)

Menu 3 chấm → **Xoá sách** → dialog confirm. Query `useBookStats` (từ `src/api/library/book-stats.ts`) hiển thị:
- Số highlights sẽ mất (kèm số user nếu > 1).
- Reading progress của user hiện tại: `trang X · Y%`.

CTA "Xuất ghi chú" gợi ý backup trước khi xoá vĩnh viễn. Sau confirm: xoá row `books` → CASCADE xoá `highlights` + `reading_progress` → xoá file PDF + cover khỏi Storage → evict blob cache IndexedDB.

### Progress bar trong khi nén

Compress qua iLovePDF không stream percent về client (server-side flow: auth → start → upload → process → download). Trước đây UI dùng `animate-pulse` (opacity flash) → chớp nháy khó chịu. Đã fix: dùng **indeterminate slider** (keyframe `indeterminate` ở `tailwind.config.ts`) — thanh nhỏ chạy trái → phải trong track, mượt, không nhấp nháy. Xem `src/components/library/UploadProgressPanel.tsx`.

---

## Summary

The BiBo Library is a full-featured shared PDF library with:
