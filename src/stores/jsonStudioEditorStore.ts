...
import { useJsonStudioStore } from './jsonStudioStore';
import { detectFormat, parseByFormat } from '@/lib/json-studio/formats';
...
const STORAGE_KEY = 'jsonStudio.editor.v1';
const LEGACY_STORAGE_KEY = 'jsonViewer.editor.v1';
...
async function commitNow(text: string) {
  ...
  if (!trimmed) {
    useJsonStudioEditorStore.setState({ parsing: false, error: null });
    return;
  }

  // Ưu tiên format đang chọn trong store; nếu user gõ format khác hẳn
  // (vd đang JSON gõ vào `<xml>`), detect lại để parse cho đúng.
  const storeFormat = useJsonStudioStore.getState().sourceFormat;
  ...
  try {
    ...
    const currentFilename = useJsonStudioStore.getState().sourceFilename;
    const currentFormat = useJsonStudioStore.getState().sourceFormat;
    useJsonStudioStore
      .getState()
    ...
    if (myToken !== commitToken) return;
    useJsonStudioEditorStore.setState({
      parsing: false,
...
function scheduleParse(text: string) {
  ...
  useJsonStudioEditorStore.setState({ parsing: true });
...
// 5 MB threshold — vượt qua thì drop raw text, chỉ persist metadata.
// sessionStorage quota là 5-10MB tuỳ browser, nên guard sớm để tránh
// quota exceeded exception + tránh serialize JSON.stringify tốn thời gian
// cho payload lớn mà user không thực sự cần restore.
const PERSIST_MAX_BYTES = 5 * 1024 * 1024;
let lastQuotaWarnAt = 0;
const QUOTA_WARN_COOLDOWN_MS = 60_000;
...
function schedulePersist(text: string) {
  ...
  persistTimer = setTimeout(() => {
    persistTimer = null;
    try {
      const { sourceFormat, sourceFilename } = useJsonStudioStore.getState();

      // Estimate byte size (UTF-16 → bytes). Actual sessionStorage cost
      // gần bằng 2x text.length trong Chrome, dùng ngưỡng lỏng để guard sớm.
      const sizeBytes = text.length * 2;
      const dropPayload = sizeBytes > PERSIST_MAX_BYTES;

      const payload: PersistedState = {
        text: dropPayload ? '' : text,
        format: sourceFormat,
      ...
      if (dropPayload) {
        const now = Date.now();
        if (now - lastQuotaWarnAt > QUOTA_WARN_COOLDOWN_MS) {
          lastQuotaWarnAt = now;
          // Import lazily để tránh circular + không blocker init nếu sonner
          // chưa mount xong ở thời điểm persist đầu tiên.
          void import('sonner').then(({ toast }) => {
            toast.warning('Input > 5MB — không lưu qua reload', {
              description: 'Chỉ metadata (format + filename) được persist.',
            });
          });
        }
      }
...
function loadPersisted(): PersistedState | null {
  try {
    // Migrate legacy sessionStorage key 1 lần (không cần flag vì sessionStorage
    // scope theo tab, migrate mỗi tab lần đầu OK).
...
function getInitialText(): string {
  ...
  if (persisted) {
    ...
    // sẽ update data thật.
    const store = useJsonStudioStore.getState();
    ...
    if (trimmed) {
      ...
      void parseByFormat(trimmed, persisted.format)
        .then((data) => {
          useJsonStudioStore.getState().setData(data, persisted.format, persisted.filename);
        })
        .catch(() => {
  ...
  const state = useJsonStudioStore.getState();
...
export const useJsonStudioEditorStore = create<EditorState>(() => ({
  text: getInitialText(),
  parsing: false,
  error: null,

  setText: (next) => {
    useJsonStudioEditorStore.setState({ text: next });
    scheduleParse(next);
  ...
  syncFromData: (text) => {
    ...
    commitToken++;
    useJsonStudioEditorStore.setState({ text, parsing: false, error: null });
