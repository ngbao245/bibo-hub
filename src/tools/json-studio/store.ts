...
import type { SourceFormat, ViewMode } from '@/tools/json-studio/lib/types';
...
export const useJsonStudioStore = create<JsonStudioState>((set) => ({
  ...
  setData: (data, format, filename) => {
    import('@/tools/json-studio/lib/calculateNodeSize').then((m) => m.clearNodeSizeCache());
    set((state) => ({
      rawData: data,
      sourceFormat: format,
  ...
  reset: () => {
    ...
    void import('@/tools/json-studio/editor-store').then(({ useJsonStudioEditorStore }) => {
