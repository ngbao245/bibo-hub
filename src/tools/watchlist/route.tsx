...
import { ArrowLeft, ListVideo, Plus, Search, X } from 'lucide-react';
...
import WatchlistCard from '@/tools/watchlist/components/WatchlistCard';
import WatchlistDialog from '@/tools/watchlist/components/WatchlistDialog';

import type { WatchlistItem, WatchlistStatus, WatchlistCategory } from '@/lib/workspace/mappers';

// ============================================================
// Watchlist Page (formerly Bookmarks / Movies)
// ============================================================
...
export default function Watchlist() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [watchlistQuery.data, statusFilter, categoryFilter, query]);
  ...
  function openEditDialog(item: WatchlistItem) {
    setEditing(item);
    setDialogOpen(true);
  }

  function handleSubmit(item: WatchlistItem | Omit<WatchlistItem, 'id' | 'createdAt' | 'updatedAt'>) {
    if ('id' in item && item.id) {
      updateItem.mutate(item as WatchlistItem, {
        onSuccess: () => {
      ...
      createItem.mutate(item as Omit<WatchlistItem, 'id' | 'createdAt' | 'updatedAt'>, {
        onSuccess: () => {
  ...
  function handleRate(item: WatchlistItem, rating: number) {
    updateItem.mutate({ ...item, rating });
  }
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
          ...
          <div className="flex items-center gap-2">
            ...
            {watchlistQuery.data && (
              <span className="font-mono text-xs text-muted-foreground">
                {filtered.length} / {watchlistQuery.data.length}
              </span>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {watchlistQuery.isLoading ? (
          ...
          <EmptyState
            icon={ListVideo}
            title={query ? `Không có kết quả cho "${query}"` : 'Chưa có mục nào'}
            description={query ? 'Thử từ khoá khác hoặc xoá bộ lọc.' : 'Thêm mục đầu tiên để bắt đầu.'}
            action={
              !query && (
