...
import type { WatchlistItem, WatchlistCategory, WatchlistStatus } from '@/lib/workspace/mappers';

// ============================================================
// WatchlistDialog — form thêm/sửa watchlist item
// ============================================================

interface WatchlistDialogProps {
  open: boolean;
  item: WatchlistItem | null; // null = tạo mới
  onClose: () => void;
  onSubmit: (item: WatchlistItem | Omit<WatchlistItem, 'id' | 'createdAt' | 'updatedAt'>) => void;
  isSubmitting?: boolean;
}

const EMPTY_ITEM: Omit<WatchlistItem, 'id' | 'createdAt' | 'updatedAt'> = {
  title: '',
  category: 'movie',
  status: 'plan',
  rating: null,
...
export default function WatchlistDialog({
  ...
  const [draft, setDraft] = useState<WatchlistItem | Omit<WatchlistItem, 'id' | 'createdAt' | 'updatedAt'>>(
    item ?? EMPTY_ITEM,
  );
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{item ? 'Sửa mục' : 'Thêm mục'}</DialogTitle>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Category">
              <select
                ...
                onChange={(e) => update('category', e.target.value as WatchlistCategory)}
        ...
        <DialogFooter>
          ...
          >
            {isSubmitting ? 'Đang lưu...' : item ? 'Cập nhật' : 'Thêm'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
