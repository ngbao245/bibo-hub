...
export default function SummaryBar({ stats }: SummaryBarProps) {
  return (
    <div className="flex flex-wrap gap-2 border-b border-border bg-card px-4 py-2">
      ...
      <Chip label="Còn hàng" value={String(stats.available)} color="text-success" />
      <Chip label="Đang về" value={String(stats.incoming)} color="text-warning" />
      <Chip label="Đã bán" value={String(stats.sold)} />
      <Chip label="Vốn giữ" value={formatMoney(stats.heldInvested)} color="text-warning" />
      <Chip
        label="Lời thực"
