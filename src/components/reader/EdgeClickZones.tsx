 ...
 *
 * Width 8% (min 48px). Hover hiện chevron và background.
 * 
 * Left zone shifts right khi sidebar open để không conflict với sidebar.
 */
function EdgeClickZones({
  onPrev,
  onNext,
  sidebarOpen = false,
}: {
  ...
  return (
    <>
      <button
        ...
        className={cn(
          'group absolute inset-y-0 flex w-[8%] min-w-[48px] items-center justify-center bg-transparent text-zinc-600 transition-all hover:bg-zinc-950/40 hover:text-zinc-100',
...
export default memo(EdgeClickZones);