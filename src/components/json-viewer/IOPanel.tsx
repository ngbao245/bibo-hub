...
import {
  ...
  parseByFormat,
  stringifyByFormat,
} from '@/lib/json-viewer/formats';
import { parseAsync } from '@/lib/json-viewer/parser-client';
import { downloadFile, getDefaultFilename } from '@/lib/json-viewer/export-formatter';
...
export function IOPanel() {
  ...
  const setData = useJsonViewerStore((s) => s.setData);
  const error = useJsonViewerEditorStore((s) => s.error);
  const editorText = useJsonViewerEditorStore.getState; // dùng getState để né subscribe
  ...
  const handleFormatChange = async (next: SourceFormat) => {
    ...
    try {
      ...
      useJsonViewerEditorStore.getState().syncFromData(converted);
    } catch (err) {
