# `components/json-viewer/` — JSON / CSV / YAML / XML Viewer UI

UI cho route `/json-viewer`. Visualize JSON, CSV, YAML, XML bằng 2 chế độ: **Graph** (Reaflow + ELK) hoặc **Tree** (pure React, lightweight).
...
- **`useJsonViewerStore`** (`stores/jsonViewerStore.ts`) — committed data (`rawData`), view mode, editor open. Graph/Tree subscribe state này.
- **`useJsonViewerEditorStore`** (`stores/jsonViewerEditorStore.ts`) — text textarea (`text`), parsing flag, error. Tách store để gõ KHÔNG trigger graph rebuild. Parse + commit sang store chính được debounce 400ms.
- **`useJsonViewerPrefsStore`** (`stores/jsonViewerPrefsStore.ts`) — preferences persist localStorage (graphTheme, zoomOnScroll, showRuler).

### Editor persistence (sessionStorage)

`jsonViewerEditorStore` tự ghi `{ text, format, filename }` vào `sessionStorage` (key `jsonViewer.editor.v1`) mỗi keystroke với debounce **800ms** (lớn hơn parse debounce 400ms để parse luôn fire trước, không tranh tài nguyên trên 1 keystroke).
...
- **[docs/json-viewer-performance.md](../../../docs/json-viewer-performance.md)** — pattern + lessons performance.
- **[src/lib/json-viewer/README.md](../../lib/json-viewer/README.md)** — parser, theme, canvas helpers.

## Editor (CodeMirror 6)

`DataEditor` dùng [CodeMirror 6](https://codemirror.net/) thay cho textarea native (đã xóa `useTextareaLineShortcuts` cũ). Setup ở [`lib/json-viewer/codemirror-setup.ts`](../../lib/json-viewer/codemirror-setup.ts).
...
Tổng tăng ~130KB gzipped trên route `/json-viewer` chunk so với textarea + regex tokenizer trước đó.
...
| Syntax highlight | `@mantine/code-highlight` + Shiki + WASM | regex 1-pass ở `lib/json-viewer/json-highlight.ts` |
...
1. Dùng `highlightJson` từ [`lib/json-viewer/json-highlight.ts`](../../lib/json-viewer/json-highlight.ts) — đã extract sẵn `tokenize` + `JSON_HIGHLIGHT_COLORS`.
...
Sửa `JSON_HIGHLIGHT_COLORS` trong `lib/json-viewer/json-highlight.ts`. 5 token type:
...
Logic export ở [`exportGraphAsImage`](../../lib/json-viewer/canvasHelpers.ts) — dùng [`html-to-image`](https://github.com/bubkoo/html-to-image) `1.11.11` (cùng version + lib mà jsoncrack dùng).
