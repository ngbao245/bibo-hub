...
import { useJsonStudioEditorStore } from '@/stores/jsonStudioEditorStore';
import {
  buildExtensions,
  getLanguageExtension,
  languageCompartment,
} from '@/lib/json-studio/codemirror-setup';
import { IOPanel } from './IOPanel';
import { cn } from '@/lib/cn';

// ============================================================
// DataEditor — CodeMirror 6 editor cho route /json-studio
// ============================================================
//
// Pattern parse debounced vẫn giữ:
//   - User gõ → CodeMirror updateListener → setText → editorStore.text
//   - editorStore debounce 400ms → parse → commit sang jsonStudioStore.rawData
...
export function DataEditor() {
  ...
  const text = useJsonStudioEditorStore((s) => s.text);
  const parsing = useJsonStudioEditorStore((s) => s.parsing);
  const error = useJsonStudioEditorStore((s) => s.error);
  const setText = useJsonStudioEditorStore((s) => s.setText);
  ...
  useEffect(() => {
    ...
    const initialText = useJsonStudioEditorStore.getState().text;
    const initialFormat = useJsonStudioStore.getState().sourceFormat;
