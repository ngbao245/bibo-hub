...
import { loadReaderCreds, loadReaderConfig, persistKey, VaultError, cacheConfig } from '@/lib/reader/vault';
...
export default function ReaderLogin() {
  ...
  async function runVault(opts: { silent?: boolean; passphrase?: string } = {}) {
    ...
    try {
      ...
      // 2. Cache config để lần sau không cần fetch lại
      cacheConfig(config);

      // 3. Load credentials và sign in
