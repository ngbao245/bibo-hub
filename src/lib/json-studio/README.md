# `lib/json-studio/` — JSON Studio logic

Pure utilities cho tool `/json-studio`. Không touch React (trừ `CollapseContext` là context object). Tách ra khỏi components để dễ test + reuse.

**Rename note**: folder cũ `lib/json-viewer/` đã đổi tên sang `lib/json-studio/` khi tool `json-viewer` rebrand thành `json-studio` (Phase 1 shell — spec `.kiro/specs/json-studio-shell/`). Route `/json-viewer` redirect sang `/json-studio` để giữ bookmark.
...
| `cache-maintenance.ts` | `clearJsonStudioCaches()` — gọi khi rời route để free RAM. |
...
Xem chi tiết: **[docs/json-studio-performance.md](../../../docs/json-studio-performance.md)**.

Tóm tắt:

1. **Parse debounce 400ms** ở `jsonStudioEditorStore`. ELK chỉ chạy sau 400ms im lặng.
2. **2 store tách biệt** — `editorStore.text` (sync mỗi keystroke) vs `studioStore.rawData` (committed).
...
- **[docs/json-studio-performance.md](../../../docs/json-studio-performance.md)** — performance patterns.
- **[src/components/json-studio/README.md](../../components/json-studio/README.md)** — components dùng helpers.